            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex gap-2 flex-shrink-0">
                <button
                  ...
                  className="p-2 rounded-lg bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50 disabled:opacity-40 disabled:cursor-not-allowed"
                  aria-label={`Regenerate document for ${item.patientName} — ${item.label}`}
                >
                  <RotateCw className={`w-4 h-4 ${item.regenerating ? 'animate-spin' : ''}`} aria-hidden="true" />
                </button>
                {item.status === 'generated' && (
                  <>
                    <button
